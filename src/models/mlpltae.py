...
class MLPLTAE(nn.Module):
    def __init__(self, input_dim=10, in_channels=128, num_classes=19, n_head=16, d_k=8, n_neurons=[256,128], dropout=0.2, d_model=256,
                 T=1000, num_steps=406, positions=None, return_att=False):
        """
        ...
        Args:
            ...
            num_steps (int, optional): Maximum sequence length, used to pre-compute the positional encoding table
        ...
        layers = []
        layers.extend([nn.Linear(self.input_dim, self.in_channels),
                       nn.BatchNorm1d(self.in_channels),
                       ])
        self.mlp_in = nn.Sequential(*layers)

        self.dropout = nn.Dropout(dropout)


        if positions is None:
            positions = num_steps + 1

        if d_model is not None:
            self.d_model = d_model
            self.inconv = nn.Sequential(nn.Conv1d(in_channels, d_model, 1),
                                        nn.LayerNorm((d_model, num_steps)))
        else:
    ...
    def forward(self, x, label, mask):
        ...
        x = self.mlp_in(x.reshape(-1, d)).reshape(sz_b, seq_len, -1)
        ...
        if self.return_att:
            ...
            return self.decoder(enc_output), label, mask
