...
class PASTIS_Dataset(tdata.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if not self.cache or item not in self.memory.keys():
            ...
            if self.target == "semantic":
            ...
            elif self.target == 'semantic_only':
                ...
                target = torch.from_numpy(semantic_ids[0].astype(int))

                if self.class_mapping is not None:
                    target = self.class_mapping(target)

            elif self.target == 'instance_only':
                semantic_ids = np.load(
                    os.path.join(
                        self.folder,
                        "ANNOTATIONS",
                ...
                target = torch.from_numpy(np.concatenate(
                        [
                            semantic_ids[0].astype(int)[None, :, :],
                            instance_ids.astype(int)[None, :, :],
                        ],
                        axis=0,
                    ))
        ...
        return (data, dates.float()), target
